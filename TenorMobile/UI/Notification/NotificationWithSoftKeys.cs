...
namespace Tenor.Mobile.UI
{
    ...
    public class NotificationWithSoftKeys : Component
    {
        ...
        private NotificationSoftKey mRightSoftKey = new NotificationSoftKey(SoftKeyType.Dismiss);
        ...
        private void ConfigureSoftKey(ref SOFTKEYNOTIFY softKey, NotificationSoftKey data, uint index)
        {
            if (data != null)
            {
                ...
                if (string.IsNullOrEmpty(data.Title))
                {
                    switch (data.Type)
                    {
                        case SoftKeyType.Dismiss:
                            title = "Dismiss";
                            break;
                        case SoftKeyType.Hide:
                            title = "Hide";
                            break;
                        case SoftKeyType.Submit:
                            title = "Submit";
                            break;
                        default:
                            break;
        ...
        public NotificationSoftKey LeftSoftKey
        {
            get
            {
                if (mLeftSoftKey == null)
                {
                    mLeftSoftKey = new NotificationSoftKey(SoftKeyType.Disabled);
                    mLeftSoftKey.Changed += new EventHandler(SoftKey_Changed);
                }
                return mLeftSoftKey;
            }
        }

        void SoftKey_Changed(object sender, EventArgs e)
        {
            RebuildSoftKeys();
        ...
        public NotificationSoftKey RightSoftKey
        {
            get
            {
                if (mRightSoftKey == null)
                {
                    mRightSoftKey = new NotificationSoftKey(SoftKeyType.Disabled);
                    mRightSoftKey.Changed += new EventHandler(SoftKey_Changed);
                }
